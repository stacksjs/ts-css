...
describe('parse + generate round-trip', () => {
  ...
  it('keeps `/*!*/` comments at generate time but strips others', () => {
    // matches css-tree behaviour: regular comments are parsed (so plugins
    // can see them) but NOT regenerated by default. `!`-comments survive.
    expect(generate(parse('/* hi */.a{color:red}'))).toBe('.a{color:red}')
    expect(generate(parse('/*! keep */.a{color:red}'))).toBe('/*! keep */.a{color:red}')
  })
